...
import { getChapters, getPrograms, getMilestones, getGrants, getUpdateLog } from '../../api/notion'
import { StatCard, CardSkeleton, SectionHeader, StatusBadge, SiadTags, ProgressBar } from '../../components/shared/UI'
import { chapterProgress, wordCountProgress, formatDate, timeAgo, formatCurrency, getStatus } from '../../utils/helpers'
import { useAuth } from '../../context/AuthContext'
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, PieChart, Pie } from 'recharts'
import { RefreshCw, ExternalLink } from 'lucide-react'

const GOLD = '#B8891A'; const TEAL = '#148585'; const CORAL = '#D4472E'; const SAGE = '#4E7D4C'
...
export default function OverviewPage() {
  ...
  const chapterChartData = chapters.map(c => ({
    name: c.props?.['Chapter Number'] || 'Ch',
    ...
    <div className="p-8 max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-ink-400 mt-1">
            PhD in Public Theology & Community Engagement — SIAD Framework
          </p>
        </div>
      ...
      <div className="flex gap-0.5 mb-8 rounded-full overflow-hidden h-1">
        {[[GOLD,40],[TEAL,25],[CORAL,20],[SAGE,15]].map(([c,w],i) => (
          <div key={i} className="h-full transition-all" style={{ width: `${w}%`, background: c }} />
        ))}
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Chapters Complete" value={`${doneChapters}/${chapters.length}`}
          sub={`${Math.round(goalWords ? (totalWords/goalWords)*100 : 0)}% of word goal`} color={GOLD} icon="📚" />
        <StatCard label="Words Written" value={totalWords.toLocaleString()}
          sub={`of ${goalWords.toLocaleString()} goal`} color={TEAL} icon="✍" />
        <StatCard label="Active Programs" value={activePrograms}
          sub={`${programs.length} total at VOW Center`} color={SAGE} icon="⛪" />
        <StatCard label="Grant Pipeline" value={formatCurrency(totalGrantAsk)}
          sub={`${formatCurrency(awardedGrants)} awarded`} color={CORAL} icon="💰" />
      </div>

      {/* Two column: Chapter progress + Recent log */}
        ...
        <div className="card p-6 animate-fade-up delay-1">
          ...
          <div className="space-y-4">
            {chapters.map(c => {
              ...
              const wPct = wordCountProgress(c.props?.['Current Word Count'], c.props?.['Word Count Goal'])
              const num = c.props?.['Chapter Number'] || ''
              return (
                <div key={c.id} className="group">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-[10px] font-mono text-ink-600 w-6 shrink-0">{num?.replace('Chapter ','Ch')?.replace('Introduction','Intro')?.replace('Conclusion','Conc')}</span>
                      <p className="text-xs text-ink-300 truncate">{c.props?.['Chapter Title'] || 'Untitled'}</p>
                    </div>
      ...
      {/* Bottom row: Programs + Grants + Log */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6 animate-fade-up delay-4">
          ...
          {grantPieData.length > 0 ? (
            <div className="flex items-center gap-4">
              <PieChart width={100} height={100}>
                <Pie data={grantPieData} cx={45} cy={45} innerRadius={28} outerRadius={45} dataKey="value" strokeWidth={0}>
                  {grantPieData.map(entry => (
                    <Cell key={entry.name} fill={pieColors[entry.name] || '#4A4A6A'} />
                  ))}
                </Pie>
              </PieChart>
            ...
            <p className="text-xs text-ink-600 text-center py-6 font-mono">No grants yet — add your first funder</p>
          )}
        </div>
        ...
        <div className="card p-6 animate-fade-up delay-5">
          ...
          <div className="space-y-3">
            {log.length === 0 && (
              <p className="text-xs text-ink-600 font-mono text-center py-4">No entries yet</p>
            )}
