...
import { Send, CheckCheck, RefreshCw, Filter } from 'lucide-react'

const SECTIONS = [
  'General','Dissertation Hub','Chapter 1','Chapter 2','Chapter 3','Chapter 4','Chapter 5',
  'VOW Center Ops','Programs','TBI','501c3 Journey','Grant Pipeline'
]

const ROLE_COLORS = {
  'Dissertation Chair': '#A8CBA6',
  'Committee Member':   '#7ACECE',
  'Ministry Leader':    '#E8C547',
  'TBI Team':          '#F2A898',
  'Grant Collaborator':'#C4C4D4',
  'Admin':             '#E8C547',
}
...
function CommentCard({ comment, isAdmin, onResolve }) {
  ...
  return (
    <div className={`card p-5 transition-opacity ${isResolved ? 'opacity-50' : ''} animate-fade-up`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-mono shrink-0"
            style={{ background: '#1A1A24', border: '1px solid #32324A' }}>
            {(p?.['Author Name'] || p?.['Author Email'] || '?')[0]?.toUpperCase()}
          </div>
          <div>
            <p className="text-xs text-ink-300 font-medium">
              {p?.['Author Name'] || p?.['Author Email'] || 'Anonymous'}
            </p>
            <p className="text-[10px] font-mono mt-0.5" style={{ color: ROLE_COLORS[p?.Role] || '#6B6B8F' }}>
              {p?.Role || 'Viewer'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isAdmin && !isResolved && (
            <button onClick={() => onResolve(comment.id)}
              className="p-1.5 rounded text-ink-600 hover:text-teal-300 transition-colors"
              title="Mark resolved">
              <CheckCheck size={13} />
            </button>
          )}
      ...
      {p?.Section && (
        <div className="mb-2">
          <span className="text-[9px] px-2 py-0.5 rounded font-mono uppercase tracking-wider"
            style={{ background: '#242432', color: '#6B6B8F', border: '1px solid #32324A' }}>
            {p.Section}
          </span>
        </div>
      )}
      ...
      {p?.Priority === 'High' && (
        <div className="mt-2 flex items-center gap-1">
          ...
          <span className="text-[9px] font-mono text-coral-400 uppercase tracking-wider">High Priority</span>
        </div>
      )}
    </div>
  )
...
export default function CommentsPage() {
  ...
  async function handleSubmit() {
    ...
    await submitComment({
      ...
      Role: user?.role === 'admin' ? 'Admin'
        : user?.role === 'chair' ? 'Dissertation Chair'
        : user?.role === 'committee' ? 'Committee Member'
        : user?.role === 'ministry' ? 'Ministry Leader'
        : 'Grant Collaborator',
      Section: section,
      Status: 'Open',
      Priority: priority,
  ...
  const filtered = comments.filter(c => {
    const sectionOk = sectionFilter === 'all' || c.props?.Section === sectionFilter
    const statusOk = statusFilter === 'all'
      || (statusFilter === 'open' && c.props?.Status !== 'Resolved')
      || (statusFilter === 'resolved' && c.props?.Status === 'Resolved')
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left: Comment form + filters + list */}
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex gap-1">
              {['open','resolved','all'].map(f => (
                <button key={f} onClick={() => setStatusFilter(f)}
                  ...
                  style={{
                    background: statusFilter===f ? 'rgba(184,137,26,0.15)' : '#1A1A24',
                    color: statusFilter===f ? '#E8C547' : '#6B6B8F',
                    border: `1px solid ${statusFilter===f ? 'rgba(184,137,26,0.3)' : '#32324A'}`,
                  }}>
                  {f}
                </button>
              ))}
            ...
            <button onClick={load} className="text-ink-600 hover:text-ink-400 transition-colors">
              <RefreshCw size={14} />
            </button>
          </div>
          ...
          {loading
            ...
            : filtered.length === 0
              ? <EmptyState icon="◌" message="No comments yet in this view" />
              : filtered.map(c => (
                  <CommentCard key={c.id} comment={c} isAdmin={isAdmin} onResolve={handleResolve} />
                ))
          }
        </div>
          ...
          {canComment ? (
            <div className="card p-5 sticky top-6">
              <SectionHeader title="Leave a Comment"
                subtitle={`As: ${user?.role === 'chair' ? 'Dissertation Chair' : user?.role}`} />
              <div className="space-y-3">
                <textarea
                  value={text}
                  onChange={e => setText(e.target.value)}
                  placeholder="Your comment, question, or feedback…"
                  rows={5}
                  ...
                  <div className="flex gap-1.5">
                    {['Normal','High','Low'].map(p => (
                      <button key={p} onClick={() => setPriority(p)}
                        ...
                        style={{
                          background: priority===p ? 'rgba(184,137,26,0.15)' : '#1A1A24',
                          color: priority===p ? '#E8C547' : '#6B6B8F',
                          border: `1px solid ${priority===p ? 'rgba(184,137,26,0.3)' : '#32324A'}`,
                        }}>
                        {p}
                      </button>
                    ))}
                ...
                {submitted ? (
                  <div className="flex items-center gap-2 text-xs text-teal-300 py-2">
                    <CheckCheck size={14} /> Comment submitted
                  </div>
                ) : (
            ...
            <div className="card p-5">
              <p className="text-xs text-ink-500 text-center leading-relaxed">
                You have read-only access. Comments can be submitted by invited collaborators.
              </p>
            </div>
          )}
